...
import useListingsStore from "./useListingStore";
import useBookingStore from "./useCalendarBookings";
    ...
    (set, get) => ({
      ...
      updateEmail: async (
        ...
        try {
          ...
          const checkResponse = await fetch(
            `http://localhost:5001/users?email=${encodeURIComponent(email)}`
          );
          ...
          const existingUsers = await checkResponse.json();
          const emailInUseByOthers = existingUsers.some(
            (user: User) => user.id !== currentUser.id && user.email === email
          );

          if (emailInUseByOthers) {
            set({
          ...
          // Update only the current user email
          const response = await fetch(
          ...
          const errorMessage =
            error instanceof Error
              ? error.message
              : "An error occurred during email update";
